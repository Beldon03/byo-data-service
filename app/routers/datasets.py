...
async def upload_dataset(file: UploadFile, conn: Connection) -> DatasetCreated:
...
@router.get("")
async def list_datasets(conn: Connection) -> list[DatasetSummary]:
    return [
...
async def get_schema(name: str, conn: Connection) -> DatasetSchema:
...
async def delete_dataset(name: str, conn: Connection) -> None:
    dataset = get_dataset_or_404(conn, name)
